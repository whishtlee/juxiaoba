...
class AccountController extends BaseController {
	...
	public function qqlogin() {
		...
		$redirect_uri = $this -> setting['site_domain'] . "/account/qq" . $type;
	...
	public function qqcallback() {
		...
		if ($_REQUEST['state'] == $_SESSION['state']) {
			...
			$aGetParam = array("grant_type" => "authorization_code", "client_id" => $this -> setting['qq_app_key'], "client_secret" => $this -> setting['qq_app_secret'], "code" => $_REQUEST["code"], "redirect_uri" => $this -> setting['site_domain'] . "/account/qqcallback");
		...
		if ($user_openid) {
			...
			if (count($user) > 0) {
				...
				if (D('user') -> where('id=' . $user['id']) -> save(array('last_login_ip' => $_SERVER['REMOTE_ADDR'], 'last_login_time' => $last_time))) {
					header('Location:' . $this -> setting['site_domain']);
				} 
			} else {
	...
	public function wblogin() {
		$type = isset($_REQUEST['type'])? $_REQUEST['type'] : 'callback';
		$_SESSION['state'] = md5(uniqid(rand(), true));
		$redirect_uri = $this -> setting['site_domain'] . "/account/wb" . $type;
		$login_url = "https://api.weibo.com/oauth2/authorize?client_id=" . $this -> setting['sina_app_key'] . "&response_type=code&redirect_uri=" . urlencode($redirect_uri) . "&state=" . $_SESSION['state'];
		header("Location:$login_url");
	} 
	public function wbcallback() {
		...
		if ($_REQUEST['state'] == $_SESSION['state']) {
			...
			$aGetParam = array("grant_type" => "authorization_code", "client_id" => $this -> setting['sina_app_key'], "client_secret" => $this -> setting['sina_app_secret'], "code" => $_REQUEST["code"], "redirect_uri" => $this -> setting['site_domain'] . "/account/wbcallback");
