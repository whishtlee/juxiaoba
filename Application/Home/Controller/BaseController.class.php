...
class BaseController extends Controller
{
    ...
    private function get_adv()
    {
        ...
        foreach ($adv_list as $val) {
            $adv[$val['name']] = htmlspecialchars_decode($val['content']);
        }
        $this->adv = $adv;
    ...
    private function get_flink()
    {
        if (S('flink')) {
            ...
            $flink_mod = D('flink');
            $flink = $flink_mod->order('ordid asc')->where('status=0')->select();
            S('flink', $flink, '3600');
        }
        $this->flink = $flink;
    ...
    protected function get_send_week_user()
    {
        $day = date('w');
        if ($day == 0) $day = 7;
        $time = strtotime(date('Y-m-d', strtotime("-{$day} day")) . ' 23:59:59');
        return $this->get_send_user($time);
    }

    private function get_send_user($time)
    {
        $joke = D('user_joke')->where('created_time >= ' . $time . ' and status=' . C('JOKE_STATUS_AUDIT'))->select();
        $user_list = array();
